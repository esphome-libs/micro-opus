...
// Header-only Ogg/Opus muxing helpers for host tests. Builds valid Ogg pages, OpusHead, and
// OpusTags so tests can synthesize an Opus stream in memory. Used by test_silent_channels and
// test_chunked.
...
inline uint32_t crc32(const uint8_t* buffer, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        crc = (crc << 8) ^ table[((crc >> 24) & 0xff) ^ buffer[i]];
    }
    return crc;
}
...
// the trailing zero-length segment Ogg requires when the packet length is an exact multiple of 255.
inline std::vector<uint8_t> make_ogg_page(uint8_t header_type, uint64_t granule_pos,
    ...
    const size_t checksum_pos = page.size();  // CRC filled in once the page is complete
    put_le32(page, 0);
...
// OpusHead for channel mapping family 0 (RTP, 1-2 channels): the common mono/stereo case.
inline std::vector<uint8_t> make_opus_head_family0(uint8_t channels, uint16_t pre_skip = 312,
                                                   uint32_t input_sample_rate = 48000) {
    std::vector<uint8_t> head;
    head.insert(head.end(), {'O', 'p', 'u', 's', 'H', 'e', 'a', 'd'});
    head.push_back(1);         // Version
    head.push_back(channels);  // Channel count
    put_le16(head, pre_skip);
    put_le32(head, input_sample_rate);
    put_le16(head, 0);  // Output gain
    head.push_back(0);  // Channel mapping family 0
    return head;
}

// OpusHead for channel mapping family 1 (Vorbis order) with an explicit mapping table.
// mapping_table must hold channels entries; value 255 marks a silent channel.
inline std::vector<uint8_t> make_opus_head_family1(uint8_t channels, uint8_t stream_count,
    ...
    put_le32(head, input_sample_rate);
    put_le16(head, 0);  // Output gain
    head.push_back(1);  // Channel mapping family 1
    head.push_back(stream_count);
    head.push_back(coupled_count);
    head.insert(head.end(), mapping_table.begin(), mapping_table.end());
    return head;
}

// Minimal valid OpusTags packet (a vendor string and zero user comments).
inline std::vector<uint8_t> make_opus_tags(const char* vendor = "micro-opus-tests") {
    std::vector<uint8_t> tags;
    tags.insert(tags.end(), {'O', 'p', 'u', 's', 'T', 'a', 'g', 's'});
    const uint32_t vendor_len = static_cast<uint32_t>(std::strlen(vendor));
    put_le32(tags, vendor_len);
    tags.insert(tags.end(), vendor, vendor + vendor_len);
    put_le32(tags, 0);  // User comment count
    return tags;
}

}  // namespace micro_opus_test

#endif  // MICRO_OPUS_TESTS_OGG_MUX_H
